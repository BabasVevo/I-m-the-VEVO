...
export async function fetchAllPendingApprovals(
  ...
  for (const exp of expenses) {
    ...
    unified.push({
      ...
      branch_name: exp.branch?.name || (exp.branch_id === 'branch-masaki' ? 'Gitega Central Branch' : 'Bujumbura Flagship (Rohero)'),
      entity_type: 'expense',
  ...
  for (const po of purchases) {
    ...
    unified.push({
      ...
      branch_name: po.branch?.name || (po.branch_id === 'branch-masaki' ? 'Gitega Central Branch' : 'Bujumbura Flagship (Rohero)'),
      entity_type: 'purchase_order',
      code: po.po_number,
